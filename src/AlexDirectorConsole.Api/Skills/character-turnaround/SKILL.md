...
7. 每次 `generate_image` 或 `edit_image` 成功后，系统会在最终回复末尾原样附加工具返回的完整 `imagePrompt`，并标明对应图片资源和“生成图片”或“修改图片”。Agent 不得手工重复、摘要或改写该提示词，避免输出两份。
...
- 不要用“本次调整”“提示词要点”等摘要替代系统自动附加的完整提示词，也不要再次手工抄写全文。

## Verification

确认图片工具返回新的媒体 Asset 和非空 `imagePrompt`；最终回复完整包含该提示词。修改图片时还需确认 `write_director_revision` 返回同一人物设定稿的新版本，且设定正文准确包含本次图片修改涉及的设定性变化。
