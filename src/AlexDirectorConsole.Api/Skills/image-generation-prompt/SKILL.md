...
7. 人物三视图固定为同一人物的正面、标准侧面、背面，三视图身份、年龄、体型、脸型、发型、服装、材质和配色一致；使用中性站姿、统一比例、完整头脚、视图不重叠，背景简洁。
8. 默认绝对禁止字幕、标题、对白文字、说明文字、水印、Logo 和任何新增文字叠加。shot 中的字幕、消息文字、输入文字、品牌标题留给后期，不要求图片模型生成可读文字。只有导演明确要求制作文字设计本身时才允许文字，并必须逐字提供内容。
9. 编辑图片只写一份完整可执行提示词：先列必须保持的原图身份、构图、姿态、光线和未修改区域，再列唯一确定的修改。禁止 `or`、`either ... or`、`and/or`、“或”“任选其一”等备选表达。
10. 所有要求必须唯一确定且内部一致。出现参考图与文字设定冲突、人物数量不一致、动作无法在单帧成立或关键信息缺失时，先报告具体冲突，不得交给生成技能。
